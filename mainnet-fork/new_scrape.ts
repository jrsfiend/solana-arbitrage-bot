...
async function setupOrca(accounts: any[], programs: any[], mints: any[], connection: web3.Connection) {
    console.log("setup orca pool")
    let orca_pid = "9W959DqEETiGZocYWCQPaJ6sBmUzgfxXfqGeTEdp3aQP"; // TODO: check ID
    ...
    orca_pool_names.forEach(name => {
        let pool = JSON.parse(fs.readFileSync(orca_pools + name, { encoding: "utf8" }));
        orca_count += 1
    ...
    console.log("get program accounts")
...
async function mercurialSetup(accounts: any[], programs: any[], mints: any[], connection: web3.Connection) {
    console.log("setup mercurial pool")
    ...
    mercurial_pool_names.forEach(n => {
        let pool = JSON.parse(fs.readFileSync(mercurial_pools + n, { encoding: "utf8" }));
        mercurial_count += 1
...
async function saberSetup(accounts: any[], programs: any[], mints: any[], connection: web3.Connection) {
    console.log("setup saber pool")
    // SABER POOL SETUP 
    ...
    saber_pool_names.forEach(name => {
        let pool = JSON.parse(fs.readFileSync(saber_pools + name, { encoding: "utf8" }));
        saber_count += 1;
        // console.log(pool)
        accounts.push(pool.pool_account)
...
async function aldrinSetup(accounts: any[], programs: any[], mints: any[], connection: web3.Connection) {
    console.log("setup aldrin pool")
    // ALDRIN POOL 
    ...
    aldrin_pool_names.forEach(name => {
        let pool = JSON.parse(fs.readFileSync(aldrin_pools + name, { encoding: "utf8" }));
        // console.log(pool)
        aldrin_count += 1;
...
async function serumSetup(accounts: any[], programs: any[], mints: any[], connection: web3.Connection) {
    console.log("setup serum amm")
    ...
    for (let name of serum_pool_names) {
        let pool = JSON.parse(fs.readFileSync(serum_pools + name, { encoding: "utf8" }));
        serum_count += 1;
...
async function main() {
    ...
    try {
        await setupOrca(accounts, programs, mints, connection);
    } catch (err) {
        console.error("orca setup error:", err)
    }

    try {
        await mercurialSetup(accounts, programs, mints, connection);
    } catch (err) {
        console.error("mercurial setup error:", err)
    }

    try {
        await saberSetup(accounts, programs, mints, connection);
    } catch (err) {
        console.error("saber setup error:", err)
    }

    try {
        await aldrinSetup(accounts, programs, mints, connection);
    } catch (err) {
        console.error("aldrin setup error:", err)
    }

    try {
        await serumSetup(accounts, programs, mints, connection);
    } catch (err) {
        console.error("serum setup error:", err)
    }
